...
pub fn Mat(r: comptime_int, c: comptime_int, comptime T: type) type {
    ...
    comptime switch (type_info) {
        .float => {},
        else => @compileError("mat not implemented for type " ++ @typeName(T)),
    };
    ...
    return struct {
        ...
        pub fn fastInverse(self: Self) Self {
            ...
            const d = 1.0 / (m[0][0] * result.data[0][0] +
        ...
        pub fn lookAt(eye: vec.Vec(3, T), target: vec.Vec(3, T), up: vec.Vec(3, T)) Self {
            comptime {
                if (r != 4 or c != 4) {
                    @compileError("lookAt is only defined for 4x4 matrices");
                }
            }
        ...
        pub fn orthographic(left: T, right: T, bottom: T, top: T, near: T, far: T) Self {
            comptime {
                if (r != 4 or c != 4) {
                    @compileError("orthographic is only defined for 4x4 matrices");
                }
            }
        ...
        pub fn perspective(fov: T, aspect: T, near: T, far: T) Self {
            comptime {
                if (r != 4 or c != 4) {
                    @compileError("perspective is only defined for 4x4 matrices");
                }
            }
        ...
        pub fn rotation(axis: vec.Vec(3, T), angle: T) Self {
            comptime {
                if (r != 4 or c != 4) {
